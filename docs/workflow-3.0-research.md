...
| 功能 | 說明 | 實現方式 | 狀態 |
|------|------|----------|------|
| **CLAUDE.md** | 全域設定入口 | 直接編輯 | ✅🔧 112→69行 |
| **Steering Documents** | 持久化上下文 | `steering/` 目錄 | ✅ |
| **- workflow.md** | D→R→T 規則、Agent 調度 | steering/workflow.md | ✅ |
| **- tech.md** | 技術棧、開發工具 | steering/tech.md | ✅ |
| **- structure.md** | 目錄結構、命名慣例 | steering/structure.md | ✅ |
| **Settings.json** | Hooks 設定 | settings.json | ✅🔧 |

### 2. Hooks（26 → 7）

| Hook | 說明 | 觸發點 | 狀態 |
|------|------|--------|------|
| **workflow-gate** | D→R→T 強制 | PreToolUse | ✅ |
| **drt-completion-checker** | 完成檢查 | Stop | ✅ |
| **subagent-validator** | 輸出驗證 | SubagentStop | ✅ |
| **openspec-complete-detector** | OpenSpec 完成檢測 | PreCompact | ✅ |
| **plugin-status-display** | 啟動顯示 | SessionStart | ✅ |
| **check-llm-service** | LLM 服務檢查 | SessionStart | ✅ |
| **session-start-cleanup** | 清理實例 | SessionStart | ✅ |
| ~~agent-start-display~~ | Agent 開始訊息 | - | 🚫 改用 CLAUDE.md |
| ~~status-display~~ | Agent 結束訊息 | - | 🚫 改用 CLAUDE.md |
| ~~parallel-opportunity-detector~~ | 並行提醒 | - | 🚫 改用 CLAUDE.md |
| ~~prompt-router~~ | 意圖偵測 | - | 🚫 |
| ~~violation-tracker~~ | 違規追蹤 | - | 🚫 |
| ~~session-report~~ | 結束報告 | - | 🚫 改用 CLAUDE.md |
| ~~state-updater~~ | 狀態更新 | - | 🚫 |
| ~~task-sync~~ | 任務同步 | - | 🚫 |
| ~~loop-manager~~ | Loop 管理 | - | 🚫 |
| ~~loop-recovery-detector~~ | Loop 恢復 | - | 🚫 |
| ~~completion-enforcer~~ | 收尾動作 | - | 🚫 |
| ~~memory hooks (5個)~~ | 記憶系統 | - | 🚫 改用 MCP |

### 3. D→R→T 工作流

| 功能 | 說明 | 實現方式 | 狀態 |
|------|------|----------|------|
| **D→R→T 強制** | 程式碼修改必須經過 R→T | Hook: workflow-gate | ✅ |
| **三種合法路徑** | Main→R→T / Design→R→T / D→R→T | steering/workflow.md | ✅ |
| **結果處理** | APPROVE→T / REJECT→D / PASS→完成 / FAIL→DEBUG | steering/workflow.md | ✅ |
| **重試限制** | max_retries=3 | steering/workflow.md | ✅ |

### 4. OpenSpec 規格驅動

| 功能 | 說明 | 實現方式 | 狀態 |
|------|------|----------|------|
| **Kanban 三階段** | specs/ → changes/ → archive/ | 目錄結構 | ✅ |
| **tasks.md 格式** | Checkbox + agent/files 元資料 | steering/structure.md | ✅ |
| **接手/恢復流程** | 從第一個 `[ ]` 繼續 | Trigger: `接手` | ✅ |
| **完成檢測** | 全部 `[x]` 觸發歸檔 | Hook: openspec-complete | ✅ |
...
| Skill | 說明 | 狀態 | 決定 |
|-------|------|------|------|
| **core** | 四大原則、D→R→T | ⏳ 待審查 | 保留 |
| **main** | 委派原則、並行化 | ⏳ 待審查 | 保留 |
| **workflow** | 流程指引、OpenSpec | ⏳ 待審查 | 保留 |
| **dev** | 開發專業知識 | ⏳ 待審查 | 保留 |
| **review** | 審查專業知識 | ⏳ 待審查 | 保留 |
| **testing** | 測試專業知識 | ⏳ 待審查 | 保留 |
| debugger | 除錯專業知識 | ⏳ | 考慮移除 |
| browser | 瀏覽器自動化 | ⏳ | 考慮移除 |
| migration | 遷移專業知識 | ⏳ | 考慮移除 |
| skill-agent | Skill/Agent 維護 | ⏳ | 考慮移除 |
| refactor | 重構專業知識 | ⏳ | 考慮移除 |
| ui | UI 視覺設計 | ⏳ | 考慮移除 |
| ux | UX 使用者體驗 | ⏳ | 考慮移除 |
| hooks-guide | Hooks 配置指南 | ⏳ | 考慮移除 |

### 7. 新功能（待實作）

| 功能 | 說明 | 實現方式 | 狀態 |
|------|------|----------|------|
| **Plan-Act-Reflect** | DEVELOPER 自我反思 | 修改 DEVELOPER agent | ⏳ |
| **零配置部署** | 自動偵測專案類型 | init.sh 腳本 | ⏳ |
| **LSP 整合** | 程式碼導航加速 | 已有 plugins | ⏳ |

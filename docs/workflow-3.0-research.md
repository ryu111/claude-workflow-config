...
├── skills/                      ← 精簡保留
│   ├── workflow/SKILL.md        - 工作流（合併 main + workflow）
│   ├── dev/SKILL.md             - 開發專業知識
│   ├── review/SKILL.md          - 審查專業知識
│   └── testing/SKILL.md         - 測試專業知識
├── agents/                      ← 精簡為 4 個核心
│   ├── architect.md
│   ├── developer.md
│   ├── reviewer.md
│   └── tester.md
├── commands/                    ← ⭐ NEW: 斜線命令觸發流程
│   ├── spec-create.md           - /spec 啟動規格流程
│   ├── review.md                - /review 觸發審查
│   └── test.md                  - /test 觸發測試
└── settings.json                ← 極簡 hooks（只保留 2-3 個）
```

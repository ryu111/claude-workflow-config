...
**為什麼 DESIGNER 不需要 REVIEWER/TESTER？**
- DESIGNER 產出的是**設計規格**，不是程式碼
- REVIEWER 審查的是程式碼品質，不是設計品質
- TESTER 測試的是程式碼功能，不是設計稿
- 設計規格的「驗證」在 DEVELOPER 實作時自然發生
- 最終 TESTER 會驗證「實作是否符合設計」

**如何判斷？**
- tasks.md 中標記 `agent: DESIGNER` → 先走 DESIGNER
- 任務描述包含「UI」「介面」「設計」「layout」→ 先走 DESIGNER
- 其他任務 → 直接走 D→R→T
